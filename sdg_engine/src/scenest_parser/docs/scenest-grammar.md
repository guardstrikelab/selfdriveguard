...
<unit> ::= deg | rad
<direction> ::= <predefined direction>|<customized direction>
<predefined direction> ::= lane <lane ID>|EGO
	// the reference system for lane <lane ID> and AV is shown as:

<customized direction> ::= “(”<real value>, <real value> [, <coordinate frame>]”)” 
...
<agent ground truth> ::= <trace state>“[truth]” “[<agent identifier>]”
<trace state> ::= <trace>“[<number>]”
...
<agent state> ::= <trace state>“[perception]”“[<agent identifier>]”

<traffic detection> ::= <trace state> [perception]“[traffic]”== <trace state> “[truth]”“[traffic]”
