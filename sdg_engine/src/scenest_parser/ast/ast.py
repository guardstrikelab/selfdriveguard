from src.scenest_parser.ast.base.motion import *
from src.scenest_parser.ast.base.pedestrian_type import *
from src.scenest_parser.ast.base.vehicle_type import *
from src.scenest_parser.ast.base.weathers import *
from src.scenest_parser.ast.base.shape import *
...
, Speed, Lane, WeatherContinuousIndex
...
class AST:
	...
	def find_node(self,name:AnyStr)->Tuple[ALLOWED_TYPES,int]:
		''' 
			finding functions.
		...
		len_of_node=len(nodes)
		for index in range(0,len_of_node):
			if name==nodes[index].get_name():
				return nodes[index],index
		else:
			raise Exception(f'undefined reference to {name}')
	def check_unique_id(self,name:AnyStr)->None:
		for val in self.get_ast_tree():
			if name == val.get_name():
				raise Exception(f'the {name} conflict with the name of type {val.get_name()}')
class ASTDumper:
	...
	def dump_position(self,indent:int,pos:Position):
		...
		if pos.has_frame():
			...
			sys.stdout.write(f'-Position:[{name}][default kind:ENU]\n')
	...
	def dump_predined_direction(self,indent:int,d:PredefinedDirection):	
		if d.is_default_ego():
			...
			if d.get_lane().is_anonymous():
				self.dump_lane(indent,d.get_lane())
			else:
				sys.stdout.write(indent*' ')
				sys.stdout.write(f'-Lane:{d.get_lane().get_name()}\n')
	def dump_customized_direction(self,indent:int,d:CustomizedDirection):
		sys.stdout.write(indent*' ')
		if d.has_frame():
			if d.has_z():
				z=d.get_z()
				if z>0:
					sys.stdout.write(f'-direction:[value:({d.get_x()},{d.get_y()},+{d.get_z()})] [kind:{CoordinateFrame.switch(d.get_frame().name)}]\n')
				elif z<0:
					sys.stdout.write(f'-direction:[value:({d.get_x()},{d.get_y()},{d.get_z()})] [kind:{CoordinateFrame.switch(d.get_frame().name)}]\n')
				else:
					sys.stdout.write(f'-direction:[value:({d.get_x()},{d.get_y()},+0)] [kind:{CoordinateFrame.switch(d.get_frame().name)}]\n')
			else:
				sys.stdout.write(f'-direction:[value:({d.get_x()},{d.get_y()})] [kind:{CoordinateFrame.switch(d.get_frame().name)}]\n')	
		else:
			if d.has_z():
				sys.stdout.write(f'-direction:[value:({d.get_x()},{d.get_y()},{d.get_z()})] [default kind:ENU]\n')
			else:
				sys.stdout.write(f'-direction:[value:({d.get_x()},{d.get_y()})] [default kind:ENU]\n')
	def dump_heading(self,indent:int,h:Heading):
		name=f'name:{h.get_name()}' if not h.is_anonymous() else f'anonymous'
		sys.stdout.write(indent*' ')
		sys.stdout.write(f'-Heading:[{name}][angle:{h.get_raw_heading_angle()} {Unit.switch(h._unit.name)}]\n')
		sys.stdout.write(indent*' ')
		if h.is_predefined_direction():
			self.dump_predined_direction(indent+2,h.get_direction())
		elif h.is_customized_direction():
			self.dump_customized_direction(indent+2,h.get_direction())
		else:
	...
	def dump_pedestrian_type(self,indent:int,t:PedestrianType):
		...
		if t.get_color().is_anonymous():
			...
			sys.stdout.write((indent+2)*' ')
			sys.stdout.write(f'-Color:{t.Color().get_name()}\n')
