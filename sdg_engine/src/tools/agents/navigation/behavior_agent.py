...
# wo tian jia de fang fa
    def set_many_destinations(self, locationlist, clean=False):
        """
        This method creates a list of waypoints from agent's position to destination location
        based on the route returned by the global router.
        ...
        for i in range(len(locationlist)-1):
            self.start_waypoint = self._map.get_waypoint(locationlist[i])# start location
            self.end_waypoint = self._map.get_waypoint(locationlist[i+1])# end location

            route_trace = self._trace_route(self.start_waypoint, self.end_waypoint)
            final_trace = final_trace + route_trace
        self._local_planner.set_global_plan(final_trace)
